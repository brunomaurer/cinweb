      ...
      <div className="container-wide">
        ...
        <div>
          <div className="flex flex-wrap justify-center gap-2 mb-8">
            {roleKeys.map((key) => (
              <button key={key} onClick={() => setActiveTab(key)} className={`px-5 py-2.5 rounded-xl text-body-sm font-medium transition-all duration-200 ${activeTab === key ? 'bg-accent-500 text-white shadow-accent' : 'bg-white text-gray-600 hover:bg-gray-50 border border-gray-150 hover:border-gray-200'}`}>{roleLabels[key]}</button>
            ))}
          </div>
                ...
                <div key={key} className="card-elevated p-6 md:p-8 animate-scale-in">
                  <div className="grid gap-5">
                    <div className="flex gap-4">
                      <div className="w-10 h-10 rounded-lg bg-red-50 flex items-center justify-center shrink-0 mt-0.5"><span className="text-red-500 font-bold text-body-sm">!</span></div>
                      <div><span className="text-caption font-semibold uppercase tracking-wider text-red-500">{labels.challenge}</span><p className="text-body text-gray-700 mt-1">{role.challenge}</p></div>
                    </div>
                    <div className="flex gap-4">
                      <div className="w-10 h-10 rounded-lg bg-accent-50 flex items-center justify-center shrink-0 mt-0.5"><Zap className="w-4 h-4 text-accent-500" /></div>
                      <div><span className="text-caption font-semibold uppercase tracking-wider text-accent-600">{labels.benefit}</span><p className="text-body text-gray-700 mt-1">{role.benefit}</p></div>
                    </div>
                    <div className="flex gap-4">
                      <div className="w-10 h-10 rounded-lg bg-green-50 flex items-center justify-center shrink-0 mt-0.5"><TrendingUp className="w-4 h-4 text-green-500" /></div>
                      <div><span className="text-caption font-semibold uppercase tracking-wider text-green-600">{labels.result}</span><p className="text-body text-gray-700 mt-1">{role.result}</p></div>
                    </div>
                  </div>
                  <div className="mt-6 pt-5 border-t border-gray-100">
                    <a href="#demo" className="inline-flex items-center gap-1.5 text-accent-600 font-semibold text-body-sm hover:text-accent-700 transition-colors group/link">{t.nav.cta}<ChevronRight className="w-4 h-4 group-hover/link:translate-x-0.5 transition-transform" /></a>
                  </div>
                </div>
              );
            })}
