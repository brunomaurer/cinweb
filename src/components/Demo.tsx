          ...
          <div className="md:col-span-3">
            {submitted ? (
              ...
              <form onSubmit={handleSubmit} className={`card-elevated p-6 md:p-8 space-y-4 ${isVisible ? 'animate-fade-up' : 'opacity-0'}`} style={{ animationDelay: '150ms' }}>
                ...
                <div>
                  <label className="input-label">{t.demo.challenge}</label>
                  <select value={form.challenge} onChange={(e) => set('challenge', e.target.value)} className="input-field" required>
                    <option value="">—</option>
                    {t.problem.options.map((opt) => (<option key={opt.id} value={opt.id}>{opt.title}</option>))}
                  </select>
                </div>
